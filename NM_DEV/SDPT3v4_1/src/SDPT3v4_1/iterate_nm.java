 ...
 *      programming, version 4.0," in Handbook on Semidefinite, Cone and
 *      Polynomial
 *      Optimization: Theory, Algorithms, Software and Applications, Anjos, M.
 *      and
 *      Lasserre, J.B., ED. Springer, 2012, pp. 715--754."
 */
public class iterate_nm<Aq> extends PrimalDualInteriorPointIterationStep1 {

    private static final String R_p = null;
    ...
    private final int dimAq;
    private final int dimAl;
    private final int m;
    private final int sigma;
    private final int sigmu;
    ...
    SDPT3v4_1(SOCPDualProblem1 problem) {
        ...
        this.indices = new int[q]; // indices[i+1] denots column index of Aq^i
        for (int i = 1, index = 1; i <= q; i++) {
            indices[i - 1] = index;
            index += problem.n(i);
        }

        this.Aq = problem.Aq_full();
        this.Al = problem.Al_full();

        this.flag_u = problem.flag_u();
        dimAq = java.util.Vector.size(Aq,2);
        dimAq = java.util.Vector.size(Al,2);
        m = java.util.Vector.size(R_p);
    ...
    public PrimalDualSolution iterate(PrimalDualSolution soln0, Vector Ax, Vector Aty, double mu) {
        ...
        Vector R_p = problem.b().minus(Ax);

        Vector zq = new SubVectorRef(soln0.s, 1, nq);
        Vector zl = new SubVectorRef(soln0.s, nq + 1, n);
        Vector Aqty = Aq.t().multiply(soln0.y);
        Vector Alty = Al.t().multiply(soln0.y);
        ...
        //par = intitparofpoint(Aq,Al,par,xl,xq,zl,zq);
        par = 
        dimAq = Size2DSyntax(Aq,2);
        dimAl = Size2DSyntax(Al,2);
        m= length(R_p);
        sigma = 0;
        sigmu = 0;
        vector par;
        [rhs, EinvRv_l, EinvRc_q,hrd] = NMrhsfun(Al, ,par,xl,xq,zl,zq,R_p, Rd_l, Rd_q,sigmu);

        schur = sparse(m,m);

        Matrix UU,EE,Afree;

        [schur,UU,EE] = schurmat_qblk_nm(dimAq,Aq,par,schur,UU,EE,par.dd_q,par.Zinv_q,xq);
        [schur,UU,EE] = schurmat_lblk_nm(dimAl,Al,par,schur,UU,EE,par.dd_l);
        [xx,coeff,L] = linsysolve_mod(par,schur,UU,Afree,EE,rhs); 
        [dXl,dXq,dy,dZl,dZq] = NMdirfun(Al,Aq,par,Rd_l,Rd_q,EinvRc_l,EinvRc_q,xl,xq,xx,m); 
        [alpha,beta,gamma,stepSize] =adjustStep(xl,xq,zl,zq,dXl,dXq,dZl,dZq);

        vector xq1 = xq.plus(alpha.multiply(dXq));
        vector xl1 = xl.plus(alpha.multiply(dXl));
        vector y1 = y.plus(beta.multiply(dy));
        vector zq1 = zq.plus(beta.multiply(dZq));
        vector zl1 = zl.plus(beta.multiply(dZl));
        ...
        Matrix Mq = problem.Aq(1).multiply(Hq[0]).multiply((problem.Aq(1)).t());
        for (int i = 2; i <= q; i++) {
            Matrix Mqi = problem.Aq(i).multiply(Hq[i - 1]).multiply((problem.Aq(i)).t());
            Mq = Mq.add(Mqi);
        }

        Matrix Ml = Al.multiply(Hl).multiply((Al).t());
        ...
        Vector AqRcq = Aq.multiply(Rc_q);
        Vector AlRcl = Al.multiply(Rc_l);

        Vector AHR_d = (Aq.multiply(HqRdq)).add(Al.multiply(HlRdl));

        // Vector hq = Aq.multiply((HqRdq.minus(Rc_q)));
        // Vector hl = Al.multiply((HlRdl.minus(Rc_l)));
 ...
 czz
        ...
        // Define search direction (dx, dy, dz), where x and z consist of "q" and "l"
        // parts.
        Vector dy = dy(L, h);

        // System.out.println("Predictor dy = " + dy);

        Vector dz_q = Rd_q.minus((Aq.t().multiply(dy)));
        Vector dz_l = Rd_l.minus((Al.t().multiply(dy)));
        ...
        // double sigma0 =
        // (soln0.x.add(dx.scaled(alpha_p))).innerProduct((soln0.s.add(dz.scaled(beta_p))))
        // / n / mu; // n denotes coln size of A_full // Q: why n?
         ...
         * Vector R_d = problem.c().minus(soln0.s).minus(Aty); // third equation, page
         * 24
         * 
         * // predictor step, Algorithm IPC, page 37
         * Vector R_c = soln0.x.scaled(-1); // seventh equation, page 24; sigma=0 and we
         * do not have the v_i^q term
         * 
         * // solving eq. 13.
         ...
         * Matrix M = A.multiply(AH.t()); // eq. 14; M = AHA' where HA' = (AH)' as H is
         * symmetric
         * 
         ...
         * // Chol chol = new Chol(denseM, true); // third paragraph,
         * "Section 5.5 Solving the Schur complement equation", page 28
         * Chol chol = new Chol(M, true); // third paragraph,
         * "Section 5.5 Solving the Schur complement equation", page 28
         * L = chol.L();
         * M = null; // free up memory for the large M
         * // denseM = null;
         ...
         * 
         * catch (Exception ex) {
         * // The algorithm stops when the matrix M is singular, hence not positive
         * semi-definite.
         * // See "Section 3.3 Stopping criteria", page 12-13.
         * 
         * System.out.println("M is not PD. The Cholesky decomposition is null.");
         ...
         * Vector dx_q = R_c.minus(multiplyHbyVector(H, ds_q)); // first equation, page
         * 25
         * 
         * 
         ...
         * Vector dx_c = corrector.minus(multiplyHbyVector(H, ds_c)); // first equation,
         * page 25
         * 
        ...
        // The implementation follows from HKMrhsfun.m in SDPT3 version 4.0 MATLAB
        // source code.

        /**
         * Step 1: Update h to h_corr.
         * 
         * h_corr = Rp + A^q * (H^q(R_d^q) - R_c^q[sigma = 0]) + A^l * (H^l(R_d^l) -
         * R_c^l[sigma = 0])
         * h_corr = Rp + A^q * (H^q(R_d^q) - \hat{R_c^q}) + A^l * (H^l(R_d^l) -
         * \hat(R_c^l))
         * where \hat{R_c^q} = R_c^q[new sigma] - CorrectedTerm-q;
         * \hat{R_c^l} = R_c^l[new sigma] - CorrectedTerm-l;
         * 
         * Therefore, h_corr = h + A^q * (R_c^q[sigma = 0] - R_c^q[new sigma] +
         * CorrectedTerm-q)
        ...
        // Vector termq_corr = Aq.multiply(Rc_q.minus(Rcq_hat));
        // Vector terml_corr = Al.multiply(Rc_l.minus(Rcl_hat));

        Vector termq_corr = Aq.multiply(Rcq_hat);
        Vector terml_corr = Al.multiply(Rcl_hat);
        ...
        Vector dzq_c = Rd_q.minus(Aq.t().multiply(dy_c));
        Vector dzl_c = Rd_l.minus(Al.t().multiply(dy_c));
        ...
        // Vector dxq_c = Rc_q.minus(multiplyHbyVector(Hq, dzq_c)); // should use
        // corrected Rc_q
        ...
        if (flag_u) {
            ...
            // The implementation follows from Line 568 - 595, sqlpmain.m in Matlab Source
            // Code.
    ...
    private void adjustStep(vector xl, vector xq, vector zl, vector zq, vector dxl, vector dxq, vector dzl,
            vector dzq) {
        alpha = scale(xl, xq, dxl, dxq);
        beta = scale(zl, zq, dzl, dzq);
        gamma = 0.9 + 0.09 * min(alpha, beta);
        stepSize = min(alpha, beta);
    }

    private Matrix sparse(int m2, int m3) {
        Matrix SparseMatrix[][] = new int[m2][m2];
     ...
     * H^q = [H_i^q]_{i\in[q]}
     * H_i^q = - (x_i^q)^T * z_i^q / gamma(z_i^q)^2 * J_i^q + x_i^q *
     * ((z_i^q)^(-1))^T + (z_i^q)^(-1) * (x_i^q)^T
     * 
     * @param xq
     * @param zq
     * @return H^q
     ...
     * Define u^(-1) = J_i^q * u / gamma^2(u), where J is negative identity matrix
     * except (1,1)-entry = 1.
     * 
     * @param u
     * @return r(u)^2, u^(-1)
    ...
    private Vector z_inv(Vector u) {
        ...
        // TODO: any more efficient way to do this? like stacking a scalar with a
        // vector? or replaceinplace for Vector?
        z_inv.set(1, u.get(1));
    ...
    private Vector Rcq_hat_HKM(double sigma, double mu, Vector x, Vector z, Vector dx, Vector dz) {
        ...
        for (int i = 1; i <= q; i++) {
            ...
            // Corrected term = rqp = hdzpM * hdxdz as in Page 2, "The calculation of
            // Mehrotra-corrector term" document.
            ...
            // Vector hdxp_r =
            // (hdxp_temp2.scaled(hdxp_temp3).add(hdxp_temp1)).scaled(gamma1);
            ...
            // MatrixFactory.replaceInPlace(hdzpM, 1, 1, 2, ni, ffpb); // TODO: fill vector
            // in "replaceInplace"
    ...
    /*
     * private Matrix[] Rcq_hat_NT(double sigma, double mu, Vector x, Vector z,
     * Vector dx, Vector dz){
     * }
     */
    ...
    private Vector Rcl_hat(double sigma, double mu, Vector x, Vector z, Vector dx, Vector dz) {
        ...
        // Note that this corrector term might suit for both HKM and NT search
        // directions.
        // TODO: double check
     ...
     * int act = 0; // the current working index to x_i[0] (the first entry in x_i)
     * in x = [x_1, x_2, ... x_q]; this is the notation used in AntoniouLu2007
     ...
     * Vector zi_inv = new DenseVector(ni); // z_i^{-1} = Jz/gamma(z)^2; J defined
     * in eq. 10 whose entries are either 1 or -1 on the diagonal
     ...
     * // Matrix term1 = new DenseMatrix(ni, ni).ONE().scaled(xsjScale); // multiply
     * by -J
     ...
     * Hi.set(1, 1, Hi.get(1, 1) - xsjScale); // add term1 efficiently, just the
     * diagonal
     * for (int j = 2; j <= ni; ++j) {
    ...
    /*
     * private Vector Mehrotra(double sigma, double mu, Vector x, Vector z, Vector
     * dx, Vector dz) {
     ...
     * 
     * // calculates hdz, line 54, HKMrhsfun.m, SDPT3 package, and line 35-41,
     * qops.m, SDPT3 package
     ...
     * }
     * // Rc - Rq = sigma*mu*z_i_inv - xi - Rq, where Rq is the Mehrotra-corrector
     * term, eq. 11.
